...
function Deploy-Environment {
    ...
    $location = if ($config -match 'location:\s*([^\s#]+)') { $matches[1] } else { "westeurope" }
    $environment = if ($config -match 'environment:\s*([^\s#]+)') { $matches[1] } else { $EnvName }
    $randomIdentifier = (New-Guid).ToString().Substring(0,3)

    # Build resource names
    $resourceGroup = "rg-${namespace}-${environment}"
    #$resourceGroup="gasunie-mlops-rg"
    $amlWorkspace = "mlw-${namespace}-${environment}"
    $keyVault = "kv${namespace}${environment}${randomIdentifier}"
    $containerRegistry = "cr${namespace}${environment}${randomIdentifier}"
    $storageAccount = "st${namespace}${environment}${randomIdentifier}"
    $appInsights = "mlw${namespace}${environment}${randomIdentifier}"
